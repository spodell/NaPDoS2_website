...
# select desired subsets of pre-trimmed aa and nucleotide sequences
	...
	# nucleotides have two cases:
		...
		# case 2a: in case of transcriptome, there can be a lot of input sequences, but not too hits
		 	# so need to generate selected trimmed nucleotide fastas, and aa fastas as a special case
	...
	# this works - don't change it
		my $num_genomic_locations = `wc -l $genomic_locations_tabfile`;
	...
	# if >2000 nucleotide domain matches originally found, just select untrimmed fastas from parent_seqs
		elsif ($num_input_seqs > $max_domain_hits)     
		{	
			# 	my $selected_id_filename = "$data_dir/$job_id"."_align_select_ids";
				...
				# need to pop these off before retrieving untrimmed sequences from original input id nums
		...
		# special case 2a
			if ($num_genomic_locations > 0 && $num_genomic_locations < 2000  && $script_source eq "subseq" )
			{
				#print "found special subset case.  Script source = $script_source.<br />";
				$orig_nuc_filename = $trim_nuc_domains_filename;
				my $select_id_numbers_file = "$data_dir/$job_id"."_select_id_nums";
				open (INFILE, "<$select_id_numbers_file") or send_err("Could not open list of selected id nums file $select_id_numbers_file<br>");
				my @selected_orfs = ();
				while (<INFILE>)
				{
					chomp;
					push @selected_orfs, $_;
				}
				close INFILE;

				# need to get id number parts from end to beginning, in case original sequence contains underscores
					# e.g. SP12BCY1_Ga0325878_1057_6_1_6_426
				foreach my $next_id (@selected_orfs)
				{
					chomp $next_id;
					#print "$next_id <br />";
				 	$selected_nucleotide_ids .= "$next_id\n";
				}
			}	
		# get trimmed nt fasta for selection
		...
		#  get trimmed aa fasta for selection, but don't over-write previously matched results
			my $selected_trim_aa_domains_filename = "$data_dir/$job_id"."_selected_aa_domain_cands.faa";
			if ($script_source eq "subseq"  )
			{
				$selected_trim_aa_domains_filename = "$data_dir/$job_id"."_selected_aa_domain_cands.faa";	
				$trimmed_aa_seq_txt = `$pks_script_dir/getseq_multiple.pl $selected_id_filename $data_dir/$job_id.fa`;
				$trimmed_aa_seq_txt .= "\n";
				&text_to_file("$trimmed_aa_seq_txt", "$selected_trim_aa_domains_filename");
			}
		}
	}
...
# if result_type = trimmed, write outstring to html <pre></pre>, end script
	if ($result_type eq "trimmed")
	{
		if ($trim_type eq "nucleic")
		{
			...
			unless (length $trimmed_nt_seq_txt > 10)
			{
				print "<pre>Sorry, couldn't find trimmed nucleotide sequences. 
Try selecting all matches instead of a domain type subset. <pre>";
...
# extract database matches to the currently selected sequences only from the master alignment file (.afa)	
	...
	# debug
		#print "extended fastas=$extended_db_fastas<br />";
...
# check to be sure 1st muscle output was successful
	unless (-s "$aln_filename")
	{
		...
		&send_err("ERROR: run_muscle line 887 can't find Muscle program alignment output file.");
	}	
